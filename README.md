...
Another improvement involved the TripDataService. Instead of placing API requests inside multiple Angular components, I centralized all calls to `/api/trips` into a single service. This reduced duplicate code and made future updates easier. If an API endpoint changes later, I only need to update the service rather than search through several different files.

---

## Testing

Testing was an important part of the development process because every layer of the application needed to work correctly. I used Postman extensively to test the `/api/trips` endpoints before connecting them to Angular. I tested GET, POST, PUT, and DELETE requests individually to make sure each one worked before moving on. This helped me identify issues in routes and controllers without having to determine whether the problem was coming from the frontend or backend.

One specific problem I ran into was that trip data was not displaying on the page even though the API was returning a 200 response. I had to trace the issue through the Angular components and eventually found that `HttpClient` was not provided in the app config file. Adding `provideHttpClient()` to `app.config.ts` fixed the issue. Debugging that problem taught me a lot about how Angular's dependency injection system works.

Security testing was another area that helped me better understand how web applications protect data. The application used JWT authentication to secure administrative functions. I tested protected routes by attempting requests without a token and confirmed that the server returned a 401 Unauthorized response. After including a valid token in the Authorization header, the requests completed successfully.

---

## Reflection

This course gave me a much better understanding of how all the pieces of a web application connect. Before CS 465, I understood basic web development concepts, but I had never built a complete application that included a frontend, backend, database, and authentication system.

The Travlr Getaways project allowed me to work with MongoDB, Express, Angular, and Node.js while seeing how each technology contributes to the overall application. Building the Angular admin page from scratch was one of the hardest parts of the project because I had to learn component structure, routing, services, and reactive forms all at the same time. But finishing it gave me a lot of confidence that I can pick up new frameworks and figure them out.
